...
class UNetPP(nn.Module):
    def __init__(self, in_channels, filters=None, deep_supervision=True):
        super().__init__()
...
class UNetBackbone(nn.Module):
    ...
    def __init__(self, in_channels = 512, branch_layer=4, filters = None, deep_supervision = True):
        super().__init__()
    ...
    def forward(self, x):
        ...
        for idx in range(len(self.branch)):
            x = self.branch[idx](self.branch_pooling[idx](x), L=3)
            out_feats += (x, )
        return out_feats